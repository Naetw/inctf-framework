...
def init_database(num_teams, services_info):
    ...
    result = run_command("mysql -u " + my_user + " -p" + MYSQL_DATABASE_PASSWORD +
                         " " + my_db + " < database.sql")
    to_return = str(result)

    my_db = MySQLdb.connect(user=my_user, passwd=MYSQL_DATABASE_PASSWORD, db=my_db,
    ...
    for i in xrange(num_teams):
        ...
        my_c.execute("""insert into teams (team_name, ip_range, created_on)
                values(%s, %s, %s)""",
        ...
        my_c.execute("""insert into team_score (team_id, score, reason, created_on)
                     values (%s, 0, 'Initial Score', %s)""",
    ...
    for team_id in team_ids:
        ...
        for service in services:
            my_c.execute("""insert into team_service_state(team_id, service_id,
                         state, reason, created_on) values (%s, %s, 2, 'Initial
                         state', %s)""", (team_id, service['id'],
    ...
    for service_info in services_info:
        try:
            ...
            if service_info['is_working'] == 1:
                ...
                service_id = create_service(service_info['name'],
                                            service_info['port'], authors_string,
                                            description, flag_id_description, c, db)
            ...
            print "service dir of " + service_info['name'] + " has wrong info.json" + str(e)


def create_service(name, port, authors, description, flag_id_description, c, db):
    db.ping()

    c.execute("""insert into services (name, port, authors, description,
              flag_id_description, created_on) values (%s, %s, %s, %s, %s, %s)""",
              (name, port, authors, description, flag_id_description,
               datetime.now().isoformat()))

    return db.insert_id()


def create_script(service_id, script_type, script, c, db):
    db.ping()
    name = script_type + ".py"
    c.execute("""insert into scripts (name, is_ours, is_bundle, type, service_id,
              is_working, created_on, latest_script) values (%s, 1, 0, %s, %s, 1, %s,
              1)""", (name, type, service_id, datetime.now().isoformat()))
    script_id = db.insert_id()
    c.execute("""insert into script_payload (script_id, payload, created_on) values
              (%s, %s, %s)""", (script_id, script, datetime.now().isoformat()))
...
if __name__ == "__main__":
    ...
    with open("/opt/database/combined_info.json") as f:
        services_combined_info = json.load(f)

    print init_database(team_count, services_combined_info)
